...
void *multiplicar(void *argumento) {
    ...
    for (int ii = dados->inicio; ii < dados->fim; ii += bloco) {
        int limite_i = ii + bloco;
        if (limite_i > dados->fim) {
            limite_i = dados->fim;
        }
...
int main(int argc, char *argv[]) {
    ...
    if (N <= 0 || T <= 0 || T > N || bloco <= 0) {
        printf("Erro: N e bloco devem ser positivos, e threads deve estar entre 1 e N.\n");
        return 1;
    }
    ...
    printf("Multiplicacao com Pthreads e blocagem\n");
    printf("N: %d\n", N);
    printf("Threads: %d\n", T);
